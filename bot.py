...
chats = {}
allowed = []
TOKEN = "1300170168:AAHb4-QQF97B9pA0EUbBdvts-XL1rz7wQ5A"
PASSWORD = "changeme"
...
if os.path.isfile('config.json'):
    with open('config.json', 'r') as f:
        config = json.load(f)
        if config['token'] == "1300170168:AAHb4-QQF97B9pA0EUbBdvts-XL1rz7wQ5A":
        ...
        if txt != "":
            if "/add " == txt[:5]:
                ...
                if len(txt_split) == 2 and "#" == txt_split[1][0]:
                    ...
                    name = ""
                    if msg['chat']['type'] == "private":
                        name = name + "Personal chat with " + msg['chat']['first_name'] + (
                            (" " + msg['chat']['last_name']) if 'last_name' in msg['chat'] else "")
                    else:
                        name = msg['chat']['title']
                ...
                if i != len(txt_split) or 'reply_to_message' in msg:
                    ...
                    for tag in tags:
                        if tag in chats:
                            ...
                            rejected.append(tag)
                    if len(rejected) > 0:
                        bot.sendMessage(chat_id, "Failed to send messages to tags <i>" + ", ".join(rejected) + "</i>",
                                        parse_mode="HTML")
                else:
